"""
    Time: 18-8-26 下午2:58
    Author: sharejing
    Description: The module implements data process strategies
...
class BRCDataset(object):
    ...
    def __init__(self, max_p_num, max_p_len, max_q_len,
        ...
        if train_files:
            ...
            self.logger.info('   (1) 训练集大小: {} question'.format(len(self.train_set)))

        if dev_files:
            for dev_file in dev_files:
                self.dev_set += self._load_dataset(dev_file)
            self.logger.info('   (2) 开发集大小: {} question'.format(len(self.dev_set)))

        if test_files:
            for test_file in test_files:
                self.test_set += self._load_dataset(test_file)
            self.logger.info('   (3) 测试集大小: {} question'.format(len(self.test_set)))

    def _load_dataset(self, data_path, train=False):
        """
        加载DuReader的trainset, devset, testset
        :param data_path:
        :param train:
        :return:
        """
    ...
    def load_external_dataset(self, sample):
        """
        加载外部百度搜索引擎得到的数据
        :param sample:
        :return:
