...
class RCModel(object):
    ...
    def _build_graph(self):
        ...
        #    (1) 训练集大小: {} question
        self.logger.info('    (1) 建立计算图所用时间: {} s'.format(time.time() - start_t))
        param_num = sum([np.prod(self.sess.run(tf.shape(v))) for v in self.all_params])
        self.logger.info('    (2) 模型中有{}个参数'.format(param_num))
    ...
    def train(self, data, epochs, batch_size, save_dir, save_prefix,
        ...
        Args:
            ...
            dropout_keep_prob: float value indicating dropout keep probability
            evaluate: whether to evaluate the model on dev set after each epoch
        """
        pad_id = self.vocab.get_id(self.vocab.pad_token)
        max_bleu_4 = 0
        for epoch in range(1, epochs + 1):
            print('\n')
            self.logger.info('       第 {} 个epoch训练模型'.format(epoch))
            train_batches = data.gen_mini_batches('train', batch_size, pad_id, shuffle=True)
            train_loss = self._train_epoch(train_batches, dropout_keep_prob)
            self.logger.info('       第 {} 个epoch的平均损失: {}'.format(epoch, train_loss))

            if evaluate:
                self.logger.info('       第 {} 个epoch后在开发集上评价模型'.format(epoch))
                if data.dev_set is not None:
                    eval_batches = data.gen_mini_batches('dev', batch_size, pad_id, shuffle=False)
                    eval_loss, bleu_rouge = self.evaluate(eval_batches)
                    self.logger.info('       开发集上损失: {}'.format(eval_loss))
                    self.logger.info('       开发集上Bleu和Rouge-L值:')
                    self.logger.info('              Bleu-1: {}'.format(bleu_rouge['Bleu-1']))
                    self.logger.info('              Bleu-2: {}'.format(bleu_rouge['Bleu-2']))
                    self.logger.info('              Bleu-3: {}'.format(bleu_rouge['Bleu-3']))
                    self.logger.info('              Bleu-4: {}'.format(bleu_rouge['Bleu-4']))
                    self.logger.info('              RougeL: {}'.format(bleu_rouge['Rouge-L']))
    ...
    def evaluate(self, eval_batches, result_dir=None, result_prefix=None, save_full_info=False):
        ...
        if result_dir is not None and result_prefix is not None:
            ...
            self.logger.info('       储存{}结果到{}'.format(result_prefix, result_file))
    ...
    def one_evaluate(self, eval_batches):
        """
        Evaluates the model performance on eval_batches and results are saved if specified
        Args:
            eval_batches: iterable batch data
    ...
    def save(self, model_dir, model_prefix):
        ...
        self.logger.info('       模型被存储在{}, with prefix {}'.format(model_dir, model_prefix))
    ...
    def restore(self, model_dir, model_prefix):
        ...
        self.logger.info('    (3) 从{}恢复模型, with prefix {}'.format(model_dir, model_prefix))
